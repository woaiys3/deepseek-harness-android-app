...
// Android 适配：@deepseek-ai/dsh-win32-process 依赖原生模块 koffi，没有 Android 版本，
// 静态 import 会让本模块在非 Windows 平台直接加载失败。它只在 Windows Job 容器路径使用，
// 因此改为仅在 win32 上按需加载；加载完成前 probeWindowsJob 的 try/catch 会把容器
// 判定为不可用并回退到常规进程管理，非 win32 平台永远不会触发这里的 import。
let win32Process;
if (process.platform === "win32") import("@deepseek-ai/dsh-win32-process").then((loaded) => {
	win32Process = loaded;
}, () => {});
function loadWin32ProcessBindings() {
	if (win32Process === void 0) throw new Error("subprocess-local: Win32 process bindings are not loaded on this platform");
	return win32Process.loadWin32ProcessBindings();
}
function probeCurrentTokenJobSupport(api) {
	if (win32Process === void 0) throw new Error("subprocess-local: Win32 Job capability probe is not loaded on this platform");
	return win32Process.probeCurrentTokenJobSupport(api);
}
...
var LocalSubprocessRuntime = class extends SubprocessRuntime {
	...
	async spawnTerminal(spec) {
		...
		let terminal;
		try {
			terminal = spawnPtyCompat(scope?.command ?? file, scope?.args ?? [...spec.argv.slice(1)], options);
		} catch (error) {
			scope?.cleanup();
			throw error;
		}
