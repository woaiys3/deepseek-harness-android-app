...
* helper cannot be spawned from pkg's virtual filesystem. Otherwise the
* platform package selected by `@vscode/ripgrep` is used — except on
* `android`, where that package does not exist at all (`@vscode/ripgrep`
* publishes darwin/win32/linux only, and its module body throws while being
* evaluated, a failure Node then caches for the rest of the process).
* Android therefore runs the `rg` shipped next to the node runtime
* (`runtime/bin/rg`), which is the same arm64 ripgrep build the payload
* already carries. Resolving at the call boundary keeps a missing or corrupt
* binary at the first search call as `SEARCH_FAILED`, rather than failing
* the Loader composition.
*
* Only a **successful** resolution is memoized: a rejected one is dropped so
* the next search retries instead of inheriting a permanent failure.
*
* @returns the ripgrep binary's absolute path.
*/
function resolveRgPath() {
	const cached = rgPathPromise;
	if (cached !== undefined) return cached;
	const pending = Promise.resolve().then(async () => {
		const explicit = process.env.DSH_RG_PATH;
		...
		// The rg shipped beside the node runtime (<runtime>/bin/rg) — always present in
		// this app's payload, and the only binary Android can reach.
		const bundled = join(executable.dir, process.platform === "win32" ? "rg.exe" : "rg");
		if (process.platform === "android" && existsSync(bundled)) return bundled;
		try {
			return (await import("@vscode/ripgrep")).rgPath;
		} catch (cause) {
			if (existsSync(bundled)) return bundled;
			throw new Error(`ripgrep is unavailable: @vscode/ripgrep publishes no package for ${process.platform}/${process.arch}, and no bundled rg was found next to the node runtime`, { cause });
		}
	});
	// Memoize successes only — a cached rejection would make every later search in
	// this process fail the same way, with no way out short of a restart.
	rgPathPromise = pending;
	pending.catch(() => { if (rgPathPromise === pending) rgPathPromise = undefined; });
	return pending;
}
/**
