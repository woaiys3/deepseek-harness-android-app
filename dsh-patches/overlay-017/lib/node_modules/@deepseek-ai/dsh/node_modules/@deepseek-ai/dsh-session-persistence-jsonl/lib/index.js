...
async function publishCurrentExclusive(staged, currentPath, internals) {
	...
	try {
		...
		/*
		 * Android 适配：/data 与 FUSE 外部存储上 hardlink 不可用（EPERM/EXDEV/ENOSYS/EACCES）。
		 * 这里**不能**退化成 rename：本函数的契约是用 EEXIST 区分"目标已存在"（返回 false），
		 * rename 会静默覆盖已有代际文件，破坏独占发布语义。
		 * 改为"独占创建 + 写入"：open(..., "wx") 在目标已存在时同样抛 EEXIST，语义等价。
		 */
		if (code === "EPERM" || code === "EXDEV" || code === "ENOSYS" || code === "EACCES") {
			let handle;
			try {
				handle = await open(currentPath, "wx");
			} catch (openError) {
				/* v8 ignore else -- the exclusive create reports collision exactly like link. */
				if (isEEXIST(openError)) return false;
				throw openError;
			}
			try {
				await handle.writeFile(await readFile(staged));
				/* 与 writeSyncedTemp 保持同一权限位（0700），避免新写文件带上宽松的默认权限。 */
...
var JsonlSessionPersistence = class extends SessionPersistence {
	...
	*/
	...
	async materializePosix(project, dir, finalPath, id, content) {
		...
		try {
			/*
			 * Android 适配：/data 分区不支持 hardlink（link() 直接报 EPERM/EXDEV），
			 * 改为 rename() 发布；目标路径已由上面的 rejectExistingLog 保证不存在，语义等价。
			 */
			if (process.platform !== "win32") {
				let viaLink = false;
				try {
					...
					const code = error instanceof Error && "code" in error ? error.code : void 0;
					if (code !== "EPERM" && code !== "EXDEV" && code !== "ENOSYS" && code !== "EACCES") throw error;
				}
				if (!viaLink) await rename(tmp, finalPath);
			} else {
				await link(tmp, finalPath);
			}
			linked = true;
