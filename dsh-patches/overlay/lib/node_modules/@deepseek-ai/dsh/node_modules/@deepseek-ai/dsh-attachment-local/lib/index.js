...
async function encode(pipeline, mediaType, quality) {
	...
	return {
		...
		/*
		 * Android 适配：**不能无条件回按请求的 mediaType 标注**。
		 * 原先这里写的是 `mediaType`，即“请求要什么就宣称输出是什么”。
		 * 在本项目的纯 JS 替身（./sharp-shim.js）下，替身能真编码的只有 PNG；
		 * 对 JPEG/WebP 它会如实透传原始字节并返回自己真实的格式。
		 * 于是旧写法会产出「声明 image/jpeg、实际是 PNG」的坏版本：
		 *   · 字节没变（与原图同一对象）时，上游那条 `data === attachment.data`
		 *     优化会跳过 verifyRequestImage()，坏版本被原样上传 → provider 400；
		 *   · 字节变了时会进 verifyRequestImage()，但它断言的是“声明 == 真实”，
		 *     于是必然抛 AttachmentError（又被 lLM 层误包成 TRANSPORT）。
		 * 本轮就是这两个分支把两个会话彻底毒死的（详见交接文档）。
		 * 现改为以**输出字节的真实格式**为准（替身通过 infoOf() 如实上报），
		 * 保证「声明 == 字节」永真。
		 */
...
/*
 * Android 适配：改用纯 JS 替身（./sharp-shim.js）而不是真正的 sharp。
 * sharp 依赖 libvips 原生二进制，官方预编译只有 darwin/linux-gnu/win32，
 * 没有 Android（bionic libc）产物 → 本项目的 payload 又按既定红线不带任何 .node/.so，
 * 所以一旦真的加载 sharp 就必然失败（图片附件整体不可用）。
 * 替身真实解析 PNG/JPEG/WebP/GIF 文件头（格式与尺寸来自真实字节），
 * 像素变换降级为“原样返回原始字节”，因此 0.1.7 新增的归一化校验
 * （depth=uchar / space=srgb / pages=1）也能对上。
 */
import sharpShim from "./sharp-shim.js";
const requireSharp = () => sharpShim;
//#endregion
...
async function syncDirectory(path) {
	...
	try {
		...
		/*
		 * Android 适配：附件存储会从 DSH_HOME 逐级向上 fsync 每个祖先目录，最终会
		 * open("/data/user/0") —— 那个目录对 App 没有 open 权限（EACCES/EPERM），
		 * 而目录持久化同步本就是尽力而为。不跳过就会让**所有附件保存/读取整体失败**
		 * （android_see / android_vscreen_see / read_image 同因报错）。
		 */
		const code = error instanceof Error && "code" in error ? error.code : void 0;
		if (code === "EACCES" || code === "EPERM") {
			/* v8 ignore next -- Android-only permission boundary; POSIX tests never hit it. */
			return;
		}
		throw error;
	}
...
async function publishImmutableAlias(root, source, target, sha256) {
	...
	try {
		...
		try {
			...
			const code = error instanceof Error && "code" in error ? error.code : void 0;
			/*
			 * Android 适配：/data 与 FUSE 外部存储上 hardlink 可能直接 EPERM/EXDEV/ENOSYS/EACCES，
			 * 不退化为拷贝就会让附件写入整体失败（0.1.5 时代已实测过）。语义不变：
			 * 目标路径已由上游的 EEXIST 分支保证不会重复发布。
			 */
			if (code === "EPERM" || code === "EXDEV" || code === "ENOSYS" || code === "EACCES") {
...
async function publishStagedObject(root, target, staged) {
	...
	try {
		...
		try {
			...
			const code = error instanceof Error && "code" in error ? error.code : void 0;
			/* Android 适配：hardlink 不可用时退化为拷贝（见 publishImmutableAlias 同款说明）。 */
			if (code === "EPERM" || code === "EXDEV" || code === "ENOSYS" || code === "EACCES") {
...
async function readRequestImageFile(root, attachment, target, signal) {
	...
	/*
	 * Android 适配：**快路径也要校验**。
	 * 原先只有“字节变了”才跑 verifyRequestImage；而 `created.data === attachment.data`
	 * （目标尺寸 ≥ 源尺寸时的直通）会直接返回，**跳过整道断言**。
	 * 在本项目的纯 JS 替身下，替身的 raw()/toBuffer() 过去不解码像素，
	 * 于是“文件头合法、内部已损坏”的图（例如凭记忆写出来的 base64 PNG）
	 * 会从入库到上传一路无阻，最后被 provider 400 —— 而且图片已进会话历史，
	 * 之后每次请求都复现，会话彻底卡死。现无条件校验一道。
	 */
	const version = cached ?? await verifyRequestImage(created, source.hasAlpha);
	signal?.throwIfAborted();
