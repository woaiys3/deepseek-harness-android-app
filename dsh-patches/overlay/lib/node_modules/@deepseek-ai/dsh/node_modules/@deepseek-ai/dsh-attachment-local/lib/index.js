...
import { chmod, rename, mkdir, open, readFile, unlink } from "node:fs/promises";
...
async function imageMetadata(data) {
	const probed = probeImageBytes(data);
	if (probed === null) throw new AttachmentError("Unsupported or malformed image data.", "INVALID_IMAGE");
	const mediaType = MEDIA_TYPES[probed.format];
...
/** Android-compatible sharp replacement: header-only raster probe (PNG/JPEG/WebP/GIF). */
...
async function probeImage(data) {
	try {
		return await imageMetadata(data);
	} catch (error) {
...
async function detectImage(data, maxPixels) {
	try {
		const detected = await imageMetadata(data);
...
var LocalAttachmentStore = class extends AttachmentStore {
	...
	constructor(ctx, config) {
		...
		this.imageLimits = Object.freeze({
			...
			// v1.5.6 修复：rc.2 内核此处漏了 maxImageDimension（schema 必填），
			// 导致带图片的会话 history 响应校验失败（history unavailable）。
			// 默认 2000 与 dsh-client-connection 的默认一致。
