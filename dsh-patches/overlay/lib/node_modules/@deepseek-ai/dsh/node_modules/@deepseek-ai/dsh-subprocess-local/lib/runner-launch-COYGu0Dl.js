...
// Android 适配：koffi 是 Windows 原生 FFI 绑定，本 payload 不含任何平台的原生二进制，
// 静态 `import koffi` 会让本模块（包括 import 它的 dsh-subprocess-local）在加载期就失败。
// koffi 只用于 Windows Job runner，因此改为“能加载则用真身，否则用惰性占位”：
// 模块可以正常加载，只有真正调用 Win32 绑定时才报错（win32 路径，本平台不可达）。
const koffi = (() => {
	const req = createRequire(import.meta.url);
	try {
		return req("koffi");
	} catch {
		return req("./koffi-stub.cjs");
	}
})();
