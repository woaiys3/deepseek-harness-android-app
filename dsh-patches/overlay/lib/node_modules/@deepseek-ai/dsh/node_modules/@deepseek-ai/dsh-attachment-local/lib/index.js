...
import sharp from "./sharp-shim.js";
...
async function encode(pipeline, mediaType, quality) {
	...
	// Android 替身编码器不做转码，返回的仍是原始字节，因此按真实字节回报，
	// 否则 mediaType/尺寸会与字节不符，后续 detectImage 校验必然失败。
	return {
...
async function syncDirectory(path) {
	...
	try {
		...
		// Android 适配：祖先目录（如 /data/user/0、/data/data）对 App 没有 open 权限
		// （EACCES/EPERM），无法 fsync。目录持久化同步是尽力而为，跳过无权限目录，
		// 否则 ensureDurableHome 的逐级向上同步会让所有附件保存/读取整体失败。
...
/**
* Android 适配：把一份内容发布到目标路径。优先硬链接（省一份 IO），但 /data 分区的
* hardlink 可能因 EPERM/EXDEV 不可用（0.1.1 会话日志补丁已实测），此时退化为复制。
* EEXIST 仍抛给调用方做完整性校验，语义不变。
* @param source - 已存在的同文件系统对象。
* @param target - 目标路径。
*/
async function publishObjectLink(source, target) {
	try {
...
async function publishImmutableAlias(root, source, target, sha256) {
	...
	try {
		...
		try {
			await publishObjectLink(source, target);
		} catch (error) {
...
async function publishStagedObject(root, target, staged) {
	...
	try {
		...
		try {
			await publishObjectLink(staged.path, target);
		} catch (error) {
			/* v8 ignore next -- Private same-filesystem directories make EEXIST the only recoverable link race. */
			if (!(error instanceof Error && "code" in error && error.code === "EEXIST")) throw error;
