...
/**
* Hard-link publication is unavailable on some filesystems, and Android refuses
* `link()` outright for app-private (f2fs + SELinux `untrusted_app`) and
* FUSE-backed targets while accepting `rename()` in the same directory. Those
* refusals carry a filesystem errno, never `EEXIST` — `EEXIST` must keep its
* meaning of "a concurrent creator won", which the no-replace contract routes to
* `FS_NOT_OBSERVED`.
*/
function isHardlinkUnsupported(error) {
	if (!(error instanceof Error) || !("code" in error)) return false;
	return error.code === "EACCES" || error.code === "EPERM" || error.code === "ENOTSUP" || error.code === "EOPNOTSUPP" || error.code === "EXDEV" || error.code === "EMLINK";
}
...
/**
* Publish a new file through `rename()` when hard links are unavailable.
* `rename()` cannot express no-replace on POSIX, so the target is re-probed
* immediately before publication: an existing file still fails closed with
* `FS_NOT_OBSERVED`, exactly as the hard-link path does. The residual race is a
* concurrent creator landing between that probe and the rename, which this
* platform cannot express without a hard link or `RENAME_NOREPLACE`.
* @param tempPath - the synced staging file to publish.
* @param absolutePath - destination that must not exist.
* @param displayPath - caller-facing path for failure messages.
* @param inspectPublicationTarget - non-following stat seam.
* @param linkError - the hard-link refusal, retained as the failure cause.
*/
async function publishCreateWithoutHardlink(tempPath, absolutePath, displayPath, inspectPublicationTarget, linkError) {
	let existing;
	try {
		existing = await inspectPublicationTarget(absolutePath);
	} catch (metadataError) {
	...
	if (existing !== void 0) {
		if (!existing.isFile()) throw new FsError(`cannot write "${displayPath}": not a regular file`, "FS_NOT_REGULAR_FILE", { cause: linkError });
		throw new FsError(`cannot overwrite existing "${displayPath}" without reading it first`, "FS_NOT_OBSERVED", { cause: linkError });
	}
...
async function writeFileAtomic(absolutePath, content, mode, signal, internals = {}, createIfAbsent) {
	...
	try {
		...
		if (createIfAbsent !== void 0) try {
			...
			if (isHardlinkUnsupported(error)) await publishCreateWithoutHardlink(tempPath, absolutePath, createIfAbsent.displayPath, inspectPublicationTarget, error);
