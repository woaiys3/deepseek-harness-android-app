...
function resolveRgPath() {
	rgPathPromise ??= Promise.resolve().then(async () => {
		...
		/*
		 * Android 适配：payload 在 <runtime>/bin/rg 里随 node 一起带了一个 rg binary
		 * （build.sh 的复制行 + MainActivity.setExecutables 每次启动 chmod +x）。
		 * 而 @vscode/ripgrep 只发布 darwin/linux-gnu/win32 平台包，**没有 android 产物**，
		 * 直接 import 在端上必然失败 —— 后果是 grep/glob 工具全部不可用（历史上反复出现过）。
		 * 所以 Android 优先用自带那个。
		 */
		...
		try {
			...
			/* 平台包缺失时的兵底：只要旁边有自带 rg 就用它，实在没有才报错。 */
