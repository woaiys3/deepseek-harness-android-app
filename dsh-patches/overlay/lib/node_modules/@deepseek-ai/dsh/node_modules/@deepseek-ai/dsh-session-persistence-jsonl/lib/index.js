		...
		for (let attempt = 0; attempt < 3; attempt += 1) {
			...
			try {
				try {
					...
					// Android 适配：bionic 下没有可用的原生 flock 绑定，flock.js 会抛
					// ERR_FLOCK_UNSUPPORTED_PLATFORM。App 内只有单个内核进程访问会话目录，
					// 这里降级为「不持锁继续」，否则发消息（写会话日志）直接失败。
					if (error?.code !== "ERR_FLOCK_UNSUPPORTED_PLATFORM") throw error;
				}
...
var JsonlSessionPersistence = class extends SessionPersistence {
	...
	*/
	...
	async materializePosix(project, dir, finalPath, id, content) {
		...
		// 改为 rename() 发布；目标路径已由上面 rejectExistingLog 保证不存在，语义等价。
		try {
