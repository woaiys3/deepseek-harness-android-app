...
import { ImageVariantId, isAttachmentError, longEdgeDimensions, requestImageDimensions } from "@deepseek-ai/dsh-attachment";
	...
	async *generate(options, connection) {
		...
		try {
			...
			try {
				...
				/*
				Android 适配（本项目）：附件层失败是确定性的本地失败（解码/编码/写盘的校验拒绝），
				与网络无关。过去一律贴成 TRANSPORT：措辞误导（去查 WiFi/代理、反复重发），
				且 TRANSPORT 在默认可重试码里（dsh-llm 的 DEFAULT_RETRYABLE_CODES）→
				一条消息白等 5 次退避重试（实测约 15 秒），失败发生在 fetch 之前，重试注定无效。
				改成：附件层错误如实归类（INVALID_REQUEST 不在可重试列表里，立即失败）。
				*/
				if (isAttachmentError(error)) throw new LlmError(`DeepSeek Messages request rejected by the attachment layer: ${detail}`, "INVALID_REQUEST", { cause: error });
				throw new LlmError(`DeepSeek Messages transport failed: ${detail}`, "TRANSPORT", { cause: error });
			} finally {
				consumer.abort();
				try {
